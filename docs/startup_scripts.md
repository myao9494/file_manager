...
- **機能**:
    ...
    - `backend/.venv_fix/Scripts/python.exe` → `backend/.venv/Scripts/python.exe` → `python` の順に Python 実行環境を自動選択します。
    - バックエンドを `8001` で起動します。
...
- Windows は仮想環境がなくても、システム `python` に依存関係が入っていれば起動できます。
- macOS/Linux は従来どおり仮想環境を利用する前提で問題ありません。
