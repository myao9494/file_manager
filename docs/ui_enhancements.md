...
|--------|------|----------|
| OFF | 検索無効（デフォルト） | なし |
| Live(Left) | リアルタイム検索 | 左ペインのパス以下 |
| Live(Right) | リアルタイム検索 | 右ペインのパス以下 |
...
インデックス検索機能が外部サービス（file_index_service:8080）に分離されたため、検索 UI から監視パス管理や再構築 UI は削除されました。
...
**設定の管理方法:**
- 監視パスや再構築は外部サービス（http://localhost:8080）のUIで管理
...
### 7. ファイル一覧検索ボックスの視認性向上 (2025-01-04)

- **デザイン変更**: ファイル一覧ペイン上部の検索ボックスの視認性を高めました。
- **背景**: ペイン背景と検索ボックスの背景色が似ており（特にダークモード時）、入力欄として認識しづらかったため。
- **変更点**:
  - 枠線をより明瞭に (`#888` / `rgba(127,127,127,0.5)`)
  - 内部シャドウ (`box-shadow: inset`) を追加し、入力欄らしさを強調
  - フォーカス時の強調表示を追加 (`blue`系のアウトライン)
  - パディングを広げ、全体的なサイズ感を調整

### 8. キーボードショートカットによるフォルダ追加 (2026-06-27)

- **内容**: 左と真ん中のペインがアクティブ（フォーカス）になっている状態で、修飾キーなしの `a` キーを押すと、自動的にフォルダ追加ダイアログ（モーダル）が表示されるようになりました。
- **目的**: ツールバーのフォルダ作成ボタンにカーソルを持っていかなくても、キーボード操作のみで素早く新規フォルダを作成できるようにしました。
- **影響範囲**: `frontend/src/components/FileList.tsx`
