...
- ファイル一覧表示
  ...
  - 左・中央ペインでは、`G` を押すとペイン内の全フォルダを並列にGit確認。Git列は作業ツリー変更を`G`、未Pushを`Push`、未Pullを`Pull`、両方を`C`で表示し、ホバーで件数・変更ファイルを確認可能。`G` はVS Codeを開き、`Push`・`Pull`・`C` は対象フォルダへのcdを含むGitコマンドをクリップボードへコピー
- 戻る / 進む / 上の階層へ移動
- ドラッグ&ドロップ
- 一括コピー / 一括移動 / 一括削除
- Safe Move（コピー → 検証 → 削除）
